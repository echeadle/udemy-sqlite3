...
class Contacts:
    ...
    def create_label_frame(self):
        ...
        Button(labelframe, text='Add Contact', 
                command=self.on_add_contact_button_clicked, bg="blue", 
    ...
    def create_bottom_buttons(self):
        Button(text='Delete Selected', 
                command=self.on_delete_selected_button_clicked,bg="red",
                fg="white").grid(row=8, column=0, sticky=W, pady=10, padx=20)
        Button(text='Modify Selected',
                command=self.on_modify_selected_button_clicked,
                bg="purple",fg="white").grid(row=8, column=1, sticky=W)
    ...
    def add_new_contact(self):
        if self.new_contacts_validated():
            ...
            parameters = (self.namefield.get(),self.emailfield.get(),
                            self.numfield.get())
    ...
    def new_contacts_validated(self):
        return len(self.namefield.get()) != 0 and len(self.namefield.get()) != 0 and len(self.namefield.get()) != 0
    ...
    def open_modify_window(self):
        ...
        Label(self.transient, text='New Phone Number:').grid(row=2, column=2)
        ...
        Button(self.transient, text='Update Contact',
                command=lambda:self.update_contacts(
                    new_phone_number_entry_widget.get(), old_number, name)).grid(
