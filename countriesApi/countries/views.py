...
def countries_list(request):
    if request.method == 'GET':
        ...
        if countries_serializer.is_valid():
            ...
            return JsonResponse(countries_serializer.data, status.HTTP_201_CREATED)
        return JsonResponse(countries_serializer.errors, status.HTTP_400_BAD_REQUEST)
...
def countries_detail(request, pk):
    ...
    if request.method == 'GET':
        countries_serializer = CountriesSerializer(country)
        return JsonResponse(countries_serializer.data)
    

    elif request.method == 'PUT':
        countries_data = JSONParder().parse(request)
